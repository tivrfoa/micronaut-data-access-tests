...
final class FieldSet<T extends FieldSet.FieldDescriptorLite<T>> {
   ...
   private void mergeFromField(Entry<T, Object> entry) {
      ...
      if (descriptor.isRepeated()) {
         ...
         if (value == null) {
            ...
            this.fields.put(descriptor, var8);
         }
      } else {
         this.fields.put(descriptor, cloneIfMutable(otherValue));
      }
   ...
   public static Object readPrimitiveField(CodedInputStream input, WireFormat.FieldType type, boolean checkUtf8) throws IOException {
      return checkUtf8
         ? WireFormat.readPrimitiveField(input, type, WireFormat.Utf8Validation.STRICT)
         : WireFormat.readPrimitiveField(input, type, WireFormat.Utf8Validation.LOOSE);
   }
   ...
   private int getMessageSetSerializedSize(Entry<T, Object> entry) {
      ...
      if (descriptor.getLiteJavaType() != WireFormat.JavaType.MESSAGE || descriptor.isRepeated() || descriptor.isPacked()) {
         ...
         return value instanceof LazyField
            ? CodedOutputStream.computeLazyFieldMessageSetExtensionSize(((FieldSet.FieldDescriptorLite)entry.getKey()).getNumber(), (LazyField)value)
            : CodedOutputStream.computeMessageSetExtensionSize(((FieldSet.FieldDescriptorLite)entry.getKey()).getNumber(), (MessageLite)value);
      }
   }
   ...
   static final class Builder<T extends FieldSet.FieldDescriptorLite<T>> {
      ...
      private static <T extends FieldSet.FieldDescriptorLite<T>> void replaceBuilders(Entry<T, Object> entry) {
         entry.setValue(replaceBuilders((T)entry.getKey(), entry.getValue()));
      }
