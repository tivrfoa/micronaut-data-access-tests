...
final class DescriptorMessageInfoFactory implements MessageInfoFactory {
   ...
   private static final Set<String> specialFieldNames = new HashSet(
      Arrays.asList(
         "Class", "DefaultInstanceForType", "ParserForType", "SerializedSize", "AllFields", "DescriptorForType", "InitializationErrorString", "CachedSize"
      )
   );
   ...
   private static StructuralMessageInfo convertProto3(Class<?> messageType, Descriptors.Descriptor messageDescriptor) {
      ...
      for(int i = 0; i < fieldDescriptors.size(); ++i) {
         ...
         if (fd.getContainingOneof() != null) {
            ...
            builder.withField(
               FieldInfo.forRepeatedMessageField(field(messageType, fd), fd.getNumber(), getFieldType(fd), getTypeForRepeatedMessageField(messageType, fd))
            );
         } else if (fd.isPacked()) {
      ...
      private DescriptorMessageInfoFactory.IsInitializedCheckAnalyzer.Node dfs(Descriptors.Descriptor descriptor) {
         ...
         for(Descriptors.FieldDescriptor field : descriptor.getFields()) {
            if (field.getJavaType() == Descriptors.FieldDescriptor.JavaType.MESSAGE) {
               DescriptorMessageInfoFactory.IsInitializedCheckAnalyzer.Node child = (DescriptorMessageInfoFactory.IsInitializedCheckAnalyzer.Node)this.nodeCache
                  .get(field.getMessageType());
               if (child == null) {
      ...
      private void analyze(DescriptorMessageInfoFactory.IsInitializedCheckAnalyzer.StronglyConnectedComponent component) {
         ...
         for(Descriptors.Descriptor descriptor : component.messages) {
            ...
            for(Descriptors.FieldDescriptor field : descriptor.getFields()) {
               ...
               if (field.getJavaType() == Descriptors.FieldDescriptor.JavaType.MESSAGE) {
                  DescriptorMessageInfoFactory.IsInitializedCheckAnalyzer.Node node = (DescriptorMessageInfoFactory.IsInitializedCheckAnalyzer.Node)this.nodeCache
                     .get(field.getMessageType());
