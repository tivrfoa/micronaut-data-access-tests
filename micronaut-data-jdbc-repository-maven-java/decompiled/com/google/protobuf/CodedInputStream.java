...
public abstract class CodedInputStream {
   ...
   public static CodedInputStream newInstance(Iterable<ByteBuffer> input) {
      return !CodedInputStream.UnsafeDirectNioDecoder.isSupported()
         ? newInstance((InputStream)(new IterableByteBufferInputStream(input)))
   ...
   static CodedInputStream newInstance(Iterable<ByteBuffer> bufs, boolean bufferIsImmutable) {
      ...
      return (CodedInputStream)(flag == 2
         ...
         : newInstance((InputStream)(new IterableByteBufferInputStream(bufs))));
   }
   ...
   private static final class ArrayDecoder extends CodedInputStream {
      ...
      public <T extends MessageLite> T readGroup(int fieldNumber, Parser<T> parser, ExtensionRegistryLite extensionRegistry) throws IOException {
         this.checkRecursionLimit();
         ++this.recursionDepth;
         T result = parser.parsePartialFrom((CodedInputStream)this, extensionRegistry);
         this.checkLastTagWas(WireFormat.makeTag(fieldNumber, 4));
         --this.recursionDepth;
         return result;
      }
   ...
   private static final class IterableDirectByteBufferDecoder extends CodedInputStream {
      ...
      public long readRawLittleEndian64() throws IOException {
         if (this.currentRemaining() >= 8L) {
            ...
            return (long)UnsafeUtil.getByte(tempPos) & 255L
               | ((long)UnsafeUtil.getByte(tempPos + 1L) & 255L) << 8
               | ((long)UnsafeUtil.getByte(tempPos + 2L) & 255L) << 16
               | ((long)UnsafeUtil.getByte(tempPos + 3L) & 255L) << 24
               | ((long)UnsafeUtil.getByte(tempPos + 4L) & 255L) << 32
               | ((long)UnsafeUtil.getByte(tempPos + 5L) & 255L) << 40
               | ((long)UnsafeUtil.getByte(tempPos + 6L) & 255L) << 48
               | ((long)UnsafeUtil.getByte(tempPos + 7L) & 255L) << 56;
         } else {
            return (long)this.readRawByte() & 255L
               | ((long)this.readRawByte() & 255L) << 8
               | ((long)this.readRawByte() & 255L) << 16
               | ((long)this.readRawByte() & 255L) << 24
               | ((long)this.readRawByte() & 255L) << 32
               | ((long)this.readRawByte() & 255L) << 40
               | ((long)this.readRawByte() & 255L) << 48
               | ((long)this.readRawByte() & 255L) << 56;
         }
      }
