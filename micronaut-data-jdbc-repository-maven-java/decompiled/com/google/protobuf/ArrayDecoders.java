...
final class ArrayDecoders {
   ...
   static long decodeFixed64(byte[] data, int position) {
      return (long)data[position] & 255L
         | ((long)data[position + 1] & 255L) << 8
         | ((long)data[position + 2] & 255L) << 16
         | ((long)data[position + 3] & 255L) << 24
         | ((long)data[position + 4] & 255L) << 32
         | ((long)data[position + 5] & 255L) << 40
         | ((long)data[position + 6] & 255L) << 48
         | ((long)data[position + 7] & 255L) << 56;
   }
            ...
            switch(extension.getLiteType()) {
               ...
               case GROUP:
                  ...
                  position = decodeGroupField(
                     Protobuf.getInstance().schemaFor(extension.getMessageDefaultInstance().getClass()), data, position, limit, endTag, registers
                  );
                  value = registers.object1;
                  break;
               case MESSAGE:
