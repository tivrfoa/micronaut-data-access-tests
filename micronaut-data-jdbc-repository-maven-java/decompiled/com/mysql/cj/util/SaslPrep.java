...
public class SaslPrep {
         ...
         for(int i = 0; i < preparedStr.length(); i = ni) {
            ...
            if (containsRAndAlCat && containsLCat) {
               throw (WrongArgumentException)ExceptionFactory.createException(
                  WrongArgumentException.class, "Cannot contain both RandALCat characters and LCat characters."
               );
            }

            if (ni >= preparedStr.length() && containsRAndAlCat && (!startsWithRAndAlCat || !endsWithRAndAlCat)) {
               throw (WrongArgumentException)ExceptionFactory.createException(
                  WrongArgumentException.class, "Cannot contain RandALCat characters and not start and end with RandALCat characters."
               );
            }
   ...
   private static boolean isProhibited(char chr, int cp) {
      return isAsciiControlCharacter(chr)
   ...
   private static boolean isNonCharacterCodePoint(int cp) {
      return cp >= 64976 && cp <= 65007
         ...
         || cp >= 917502 && cp <= 917503
         || cp >= 983038 && cp <= 983039
         || cp >= 1048574 && cp <= 1048575
         || cp >= 1114110 && cp <= 1114111;
   }
