...
public class InsertParams {
   ...
   public void setProjection(String[] projection) {
      this.projection = (List)Arrays.stream(projection).map(p -> new ExprParser(p, true).parseTableInsertField()).collect(Collectors.toList());
   }
   ...
   public void addRow(List<Object> row) {
      this.rows
         .add(
            MysqlxCrud.Insert.TypedRow.newBuilder()
               .addAllField((Iterable<? extends MysqlxExpr.Expr>)row.stream().map(f -> ExprUtil.argObjectToExpr(f, true)).collect(Collectors.toList()))
               .build()
         );
   }
