...
public class XDevApiConnectionUrl extends ConnectionUrl {
   ...
   public XDevApiConnectionUrl(ConnectionUrlParser connStrParser, Properties info) {
      ...
      if (hasPriority) {
         ...
         this.hosts.sort(Comparator.comparing(hix -> Integer.parseInt((String)hix.getHostProperties().get(PropertyKey.PRIORITY.getKeyName()))).reversed());
      }
   ...
   public List<HostInfo> getHostsList(HostsListView view) {
      if (this.prioritySorted) {
         if (this.hasDuplicatedPriorities) {
            Map<Integer, List<HostInfo>> hostsByPriority = (Map)this.hosts
               .stream()
               .collect(Collectors.groupingBy(hi -> Integer.valueOf((String)hi.getHostProperties().get(PropertyKey.PRIORITY.getKeyName()))));
            this.hosts = (List)hostsByPriority.entrySet()
               .stream()
