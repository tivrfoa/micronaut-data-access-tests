...
public abstract class AbstractRuntimeProperty<T> implements RuntimeProperty<T>, Serializable {
   ...
   public void setValue(T value, ExceptionInterceptor exceptionInterceptor) {
      if (this.getPropertyDefinition().isRuntimeModifiable()) {
         ...
         throw (PropertyNotModifiableException)ExceptionFactory.createException(
            PropertyNotModifiableException.class,
            Messages.getString("ConnectionProperties.dynamicChangeIsNotAllowed", new Object[]{"'" + this.getPropertyDefinition().getName() + "'"})
         );
      }
   }
}
