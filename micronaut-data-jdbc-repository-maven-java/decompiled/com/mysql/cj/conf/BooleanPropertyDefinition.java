...
public class BooleanPropertyDefinition extends AbstractPropertyDefinition<Boolean> {
   ...
   public BooleanPropertyDefinition(
      PropertyKey key, Boolean defaultValue, boolean isRuntimeModifiable, String description, String sinceVersion, String category, int orderInCategory
   ) {
   ...
   public static Boolean booleanFrom(String name, String value, ExceptionInterceptor exceptionInterceptor) {
      try {
         ...
         throw ExceptionFactory.createException(
            Messages.getString(
               "PropertyDefinition.1", new Object[]{name, StringUtils.stringArrayToString(getBooleanAllowableValues(), "'", "', '", "' or '", "'"), value}
            ),
            var4,
   ...
   public static String[] getBooleanAllowableValues() {
      return (String[])Arrays.stream(BooleanPropertyDefinition.AllowableValues.values()).map(Enum::toString).toArray(x$0 -> new String[x$0]);
   }
