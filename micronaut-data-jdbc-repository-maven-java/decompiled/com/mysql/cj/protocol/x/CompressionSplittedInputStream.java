...
public class CompressionSplittedInputStream extends FilterInputStream {
   ...
   public int available() throws IOException {
      this.ensureOpen();
      return this.compressorIn != null
         ? this.compressorIn.available()
         : (this.frameHeaderConsumed > 0 ? 5 - this.frameHeaderConsumed : 0) + this.in.available();
   }
   ...
   public int read() throws IOException {
      ...
      return read >= 0 ? this.singleByte[0] & 0xFF : read;
   }
   ...
   private MysqlxConnection.Compression parseCompressedMessage() {
      ...
      try {
         ...
         throw (CJCommunicationsException)ExceptionFactory.createException(
            CJCommunicationsException.class, Messages.getString("Protocol.Compression.Streams.0"), var5
         );
      }
      ...
      try {
         ...
         throw (WrongArgumentException)ExceptionFactory.createException(
            WrongArgumentException.class, Messages.getString("Protocol.Compression.Streams.1"), var4
         );
      }
   }

   private boolean isDataAvailable() throws IOException {
      return this.isCompressedDataAvailable()
         || this.frameHeaderConsumed > 0 && this.frameHeaderConsumed < 5
         || this.isFrameHeaderFullyConsumed() && this.framePayloadConsumed < this.framePayloadLength;
   }
