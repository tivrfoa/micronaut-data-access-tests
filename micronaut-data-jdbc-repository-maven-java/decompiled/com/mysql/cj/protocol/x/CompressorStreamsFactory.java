...
public class CompressorStreamsFactory {
   ...
   public InputStream getInputStreamInstance(InputStream in) {
      ...
      InputStream compressionIn = (InputStream)Util.getInstance(
         ...
         null,
         Messages.getString(
            "Protocol.Compression.IoFactory.0", new Object[]{this.compressionAlgorithm.getInputStreamClass().getName(), this.compressionAlgorithm}
         )
      );
      if (this.areCompressedStreamsContinuous()) {
   ...
   public OutputStream getOutputStreamInstance(OutputStream out) {
      ...
      OutputStream compressionOut = (OutputStream)Util.getInstance(
         this.compressionAlgorithm.getOutputStreamClass().getName(),
         new Class[]{OutputStream.class},
         new Object[]{underlyingOut},
         null,
         Messages.getString(
            "Protocol.Compression.IoFactory.1", new Object[]{this.compressionAlgorithm.getOutputStreamClass().getName(), this.compressionAlgorithm}
         )
      );
      if (this.areCompressedStreamsContinuous()) {
