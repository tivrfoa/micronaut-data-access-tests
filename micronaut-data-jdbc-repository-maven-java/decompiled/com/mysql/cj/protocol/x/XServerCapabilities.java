...
public class XServerCapabilities implements ServerCapabilities {
   ...
   public List<String> getAuthenticationMechanisms() {
      return (List<String>)((MysqlxDatatypes.Any)this.capabilities.get(KEY_AUTHENTICATION_MECHANISMS))
         .getArray()
         .getValueList()
         .stream()
   ...
   public Map<String, List<String>> getCompression() {
      return this.hasCapability(KEY_COMPRESSION)
         ? (Map)((MysqlxDatatypes.Any)this.capabilities.get(KEY_COMPRESSION))
            .getObj()
            .getFldList()
            .stream()
            .collect(
               Collectors.toMap(
                  ...
                  f -> (List)f.getValue()
                        ...
                        .getValueList()
                        .stream()
                        .map(v -> v.getScalar().getVString().getValue().toStringUtf8().toLowerCase())
                        .collect(Collectors.toList())
               )
            )
