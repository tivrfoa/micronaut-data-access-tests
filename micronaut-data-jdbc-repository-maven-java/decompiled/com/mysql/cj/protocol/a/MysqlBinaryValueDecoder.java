...
public class MysqlBinaryValueDecoder implements ValueDecoder {
   ...
   public <T> T decodeTimestamp(byte[] bytes, int offset, int length, int scale, ValueFactory<T> vf) {
      if (length == 0) {
         ...
         int seconds = 0;
         int nanos = 0;
         year = bytes[offset + 0] & 255 | (bytes[offset + 1] & 255) << 8;
   ...
   public <T> T decodeTime(byte[] bytes, int offset, int length, int scale, ValueFactory<T> vf) {
      if (length == 0) {
         ...
         days = bytes[offset + 1] & 255 | (bytes[offset + 2] & 255) << 8 | (bytes[offset + 3] & 255) << 16 | (bytes[offset + 4] & 255) << 24;
   ...
   public <T> T decodeInt8(byte[] bytes, int offset, int length, ValueFactory<T> vf) {
      if (length != 8) {
         ...
         long asLong = (long)(bytes[offset] & 255)
   ...
   public <T> T decodeDouble(byte[] bytes, int offset, int length, ValueFactory<T> vf) {
      if (length != 8) {
         ...
         long valueAsLong = (long)(bytes[offset + 0] & 255)
            | (long)(bytes[offset + 1] & 255) << 8
            | (long)(bytes[offset + 2] & 255) << 16
            | (long)(bytes[offset + 3] & 255) << 24
            | (long)(bytes[offset + 4] & 255) << 32
