...
public class CompressedPacketSender implements MessageSender<NativePacketPayload> {
   ...
   private void addUncompressedHeader(byte packetSequence, int uncompressedPacketLen) {
      ...
      this.compressedPayloadLen += this.deflater
         .deflate(this.compressedPacket, this.compressedPayloadLen, this.compressedPacket.length - this.compressedPayloadLen);
   }
   ...
   private void sendCompressedPacket(int uncompressedPayloadLen) throws IOException {
      this.writeCompressedHeader(this.compressedPayloadLen, this.compressedSequenceId++, uncompressedPayloadLen);
      ...
      if (packetLen < 50) {
         ...
         while(true) {
            ...
            if (this.compressedPayloadLen >= len) {
               this.writeCompressedHeader(unsentPayloadLen + len, this.compressedSequenceId++, 0);
               this.outputStream.write(packet, unsentOffset, unsentPayloadLen);
