...
public class LocalTimeValueEncoder extends AbstractValueEncoder {
   ...
   public String getString(BindValue binding) {
      switch(binding.getMysqlType()) {
         ...
         case TIME: {
            ...
            sb.append(
               this.adjustLocalTime(LocalTime.ofNanoOfDay(((LocalTime)binding.getValue()).toNanoOfDay()), binding.getField())
         ...
         case LONGTEXT: {
            ...
            sb.append(
               ((LocalTime)binding.getValue())
                  .format(
                     this.sendFractionalSeconds.getValue() && ((LocalTime)binding.getValue()).getNano() > 0
   ...
   public void encodeAsBinary(Message msg, BindValue binding) {
      ...
      switch(binding.getMysqlType()) {
         case TIME:
            this.writeTime(
               msg, InternalTime.from(this.adjustLocalTime(LocalTime.ofNanoOfDay(((LocalTime)binding.getValue()).toNanoOfDay()), binding.getField()))
            );
            return;
         case CHAR:
         case VARCHAR:
