   ...
   protected class ConnectionErrorFiringInvocationHandler implements InvocationHandler {
      ...
      private Object proxyIfInterfaceIsJdbc(Object toProxy, Class<?> clazz) {
         ...
         if (var6 < var5) {
            ...
            return !"java.sql".equals(packageName) && !"javax.sql".equals(packageName)
               ? this.proxyIfInterfaceIsJdbc(toProxy, iclass)
               : Proxy.newProxyInstance(toProxy.getClass().getClassLoader(), interfaces, WrapperBase.this.new ConnectionErrorFiringInvocationHandler(toProxy));
         } else {
            return toProxy;
         }
      }
