            ...
            if (fields.length <= 0) {
               ...
               if (singleTableName == null) {
                  ...
                  if (this.getSession().getPropertySet().getBooleanProperty(PropertyKey.strictUpdates).getValue()) {
                     ...
                     try {
                        rs = this.session.getPropertySet().getEnumProperty(PropertyKey.databaseTerm).getValue() == PropertyDefinitions.DatabaseTerm.SCHEMA
                     ...
                     if (!this.isUpdatable) {
                        this.notUpdatableReason = existingPrimaryKeysCount > 1
   ...
   private void extractDefaultValues() throws SQLException {
      ...
      for(Entry<String, Map<String, Map<String, Integer>>> dbEntry : this.databasesUsedToTablesUsed.entrySet()) {
         for(Entry<String, Map<String, Integer>> tableEntry : ((Map)dbEntry.getValue()).entrySet()) {
            ...
            try {
               columnsResultSet = this.session.getPropertySet().getEnumProperty(PropertyKey.databaseTerm).getValue() == PropertyDefinitions.DatabaseTerm.SCHEMA
      ...
      try {
         if (!this.isUpdatable) {
            ...
            for(int i = 0; i < fields.length; ++i) {
               ...
               String databaseName = fields[i].getDatabaseName();
               String qualifiedColumnName = StringUtils.getFullyQualifiedName(databaseName, tableName, quotedId, this.pedantic)
                  + '.'
                  + StringUtils.quoteIdentifier(columnName, quotedId, this.pedantic);
               if (fields[i].isPrimaryKey()) {
         ...
         if (!this.isClosed) {
            synchronized(this.checkClosed().getConnectionMutex()) {
               ...
               if (this.useUsageAdvisor && this.deleter == null && this.inserter == null && this.refresher == null && this.updater == null) {
                  this.eventSink
                     .processEvent((byte)0, this.session, this.getOwningStatement(), this, 0L, new Throwable(), Messages.getString("UpdatableResultSet.34"));
               }
