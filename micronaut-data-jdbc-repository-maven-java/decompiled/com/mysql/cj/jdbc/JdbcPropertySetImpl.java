...
public class JdbcPropertySetImpl extends DefaultPropertySet implements JdbcPropertySet {
   ...
   public List<DriverPropertyInfo> exposeAsDriverPropertyInfo() throws SQLException {
      return (List<DriverPropertyInfo>)PropertyDefinitions.PROPERTY_KEY_TO_PROPERTY_DEFINITION
         .entrySet()
         .stream()
         .filter(e -> !((PropertyDefinition)e.getValue()).getCategory().equals(PropertyDefinitions.CATEGORY_XDEVAPI))
         .map(Entry::getKey)
