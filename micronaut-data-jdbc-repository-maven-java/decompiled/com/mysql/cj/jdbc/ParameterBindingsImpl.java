...
public class ParameterBindingsImpl implements ParameterBindings {
   ...
   ParameterBindingsImpl(PreparedQuery query, Session session, ResultSetFactory resultSetFactory) throws SQLException {
      ...
      for(int i = 0; i < paramCount; ++i) {
         ...
         switch(this.queryBindings.getBindValues()[i].getMysqlType()) {
            ...
            default:
               try {
                  charsetIndex = session.getServerSession()
                     .getCharsetSettings()
                     .getCollationIndexForJavaEncoding(
                        (String)this.propertySet.getStringProperty(PropertyKey.characterEncoding).getValue(), session.getServerSession().getServerVersion()
                     );
               } catch (RuntimeException var12) {
                  throw SQLError.createSQLException(var12.toString(), "S1009", var12, null);
               }
         }
         ...
         Field parameterMetadata = new Field(
            ...
            "parameter_" + (i + 1),
            charsetIndex,
            (String)this.propertySet.getStringProperty(PropertyKey.characterEncoding).getValue(),
   ...
   private byte[] getBytesRepresentationForBatch(int parameterIndex, int commandIndex) {
      ...
      return batchedArg instanceof String
         ? StringUtils.getBytes((String)batchedArg, (String)this.propertySet.getStringProperty(PropertyKey.characterEncoding).getValue())
         : ((QueryBindings)batchedArg).getBytesRepresentation(parameterIndex);
   }
