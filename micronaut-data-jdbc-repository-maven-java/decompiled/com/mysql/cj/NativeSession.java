...
public class NativeSession extends CoreSession implements Serializable {
            ...
            try {
               ...
               throw ExceptionFactory.createException(
                  Messages.getString(
                     ...
                     new Object[]{this.getPropertySet().getStringProperty(PropertyKey.queryInfoCacheFactory).getValue(), PropertyKey.queryInfoCacheFactory}
                  ),
                  var7,
      ...
      try {
         ...
         if (this.versionMeetsMinimum(5, 1, 0)) {
            ...
            NativePacketPayload resultPacket = (NativePacketPayload)this.protocol
               .sendCommand(this.commandBuilder.buildComQuery(null, queryBuf.toString()), false, 0);
   ...
   private String findProcessHost(long threadId) {
      try {
         ...
         NativePacketPayload resultPacket = !this.versionMeetsMinimum(5, 6, 0) || ps == null || !"1".contentEquals(ps) && !"ON".contentEquals(ps)
            ? (NativePacketPayload)this.protocol.sendCommand(this.commandBuilder.buildComQuery(null, "SHOW PROCESSLIST"), false, 0)
            : (NativePacketPayload)this.protocol
               .sendCommand(
                  this.commandBuilder
                     .buildComQuery(
                        null, "select PROCESSLIST_ID, PROCESSLIST_USER, PROCESSLIST_HOST from performance_schema.threads where PROCESSLIST_ID=" + threadId
                     ),
                  false,
                  0
               );
   ...
   public String queryServerVariable(String varName) {
      try {
         NativePacketPayload resultPacket = (NativePacketPayload)this.protocol
            .sendCommand(this.commandBuilder.buildComQuery(null, "SELECT " + varName), false, 0);
         Resultset rs = ((NativeProtocol)this.protocol)
            .readAllResults(-1, false, resultPacket, false, null, new ResultsetFactory(Resultset.Type.FORWARD_ONLY, null));
         ValueFactory<String> svf = new StringValueFactory(this.propertySet);
         Row r;
   ...
   public <T extends Resultset> T execSQL(
      ...
      long queryStartTime = this.gatherPerfMetrics.getValue() ? System.currentTimeMillis() : 0L;
      ...
      try {
         ex = packet == null
            ? ((NativeProtocol)this.protocol)
               .sendQueryString(callingQuery, query, (String)this.characterEncoding.getValue(), maxRows, streamResults, cachedMetadata, resultSetFactory)
         ...
         if (this.getPropertySet().getBooleanProperty(PropertyKey.dumpQueriesOnException).getValue()) {
            String extractedSql = NativePacketPayload.extractSqlFromPacket(
               query, packet, endOfQueryPacketPosition, this.getPropertySet().getIntegerProperty(PropertyKey.maxQuerySizeToLog).getValue()
            );
   ...
   public void ping(boolean checkForClosedConnection, int timeoutMillis) {
      ...
      long pingMillisLifetime = (long)((Integer)this.getPropertySet().getIntegerProperty(PropertyKey.selfDestructOnPingSecondsLifetime).getValue()).intValue();
