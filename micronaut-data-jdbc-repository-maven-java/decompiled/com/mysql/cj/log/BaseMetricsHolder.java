...
public class BaseMetricsHolder {
   ...
   private void addToHistogram(
      int[] histogramCounts, long[] histogramBreakpoints, long value, int numberOfTimes, long currentLowerBound, long currentUpperBound
   ) {
   ...
   private void addToPerformanceHistogram(long value, int numberOfTimes) {
      ...
      this.addToHistogram(
         ...
         value,
         numberOfTimes,
         this.shortestQueryTimeMs == Long.MAX_VALUE ? 0L : this.shortestQueryTimeMs,
   ...
   private void addToTablesAccessedHistogram(long value, int numberOfTimes) {
      ...
      this.addToHistogram(
         ...
         value,
         numberOfTimes,
         this.minimumNumberTablesAccessed == Long.MAX_VALUE ? 0L : this.minimumNumberTablesAccessed,
   ...
   public boolean checkAbonormallyLongQuery(long millisOrNanos) {
      ...
      if (this.queryTimeCount > 14L) {
         double stddev = Math.sqrt(
            (this.queryTimeSumSquares - this.queryTimeSum * this.queryTimeSum / (double)this.queryTimeCount) / (double)(this.queryTimeCount - 1L)
         );
