...
public class StringValueFactory implements ValueFactory<String> {
   ...
   public String createFromTimestamp(InternalTimestamp its) {
      return String.format(
         "%s %s",
         this.createFromDate(its),
         this.createFromTime(new InternalTime(its.getHours(), its.getMinutes(), its.getSeconds(), its.getNanos(), its.getScale()))
      );
   }
   ...
   public String createFromBytes(byte[] bytes, int offset, int length, Field f) {
      return StringUtils.toString(
         bytes, offset, length, f.getCollationIndex() == 63 ? (String)this.pset.getStringProperty(PropertyKey.characterEncoding).getValue() : f.getEncoding()
      );
   }
