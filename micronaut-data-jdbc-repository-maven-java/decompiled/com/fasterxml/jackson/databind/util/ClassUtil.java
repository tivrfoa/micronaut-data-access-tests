...
public final class ClassUtil {
   ...
   public static void checkAndFixAccess(Member member, boolean evenIfAlreadyPublic) {
      ...
      try {
         ...
         if (!ao.isAccessible()) {
            ...
            throw new IllegalArgumentException(
               "Cannot access " + member + " (from class " + declClass.getName() + "; failed to set access: " + var5.getMessage()
            );
         }
   ...
   public static Method[] getClassMethods(Class<?> cls) {
      try {
         ...
         if (loader == null) {
            return _failGetClassMethods(cls, var7);
         } else {
            Class<?> contextClass;
            try {
            ...
            try {
               ...
               return _failGetClassMethods(cls, var5);
            }
         }
      } catch (Throwable var8) {
         return _failGetClassMethods(cls, var8);
      }
   }
   ...
   private static Method[] _failGetClassMethods(Class<?> cls, Throwable rootCause) throws IllegalArgumentException {
      throw new IllegalArgumentException(
         String.format(
            "Failed on call to `getDeclaredMethods()` on class `%s`, problem: (%s) %s", cls.getName(), rootCause.getClass().getName(), rootCause.getMessage()
         ),
         rootCause
      );
   }
