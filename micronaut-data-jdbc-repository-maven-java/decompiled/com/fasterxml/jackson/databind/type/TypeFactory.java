...
public class TypeFactory implements Serializable {
   ...
   protected TypeFactory(LRUMap<Object, JavaType> typeCache, TypeParser p, TypeModifier[] mods, ClassLoader classLoader) {
      this(typeCache, p, mods, classLoader);
   }
   ...
   public JavaType constructSpecializedType(JavaType baseType, Class<?> subclass, boolean relaxedCompatibilityCheck) throws IllegalArgumentException {
      ...
      if (rawBase == subclass) {
         ...
         if (rawBase == Object.class) {
            newType = this._fromClass(null, subclass, EMPTY_BINDINGS);
         } else {
            if (!rawBase.isAssignableFrom(subclass)) {
               throw new IllegalArgumentException(
                  String.format("Class %s not subtype of %s", ClassUtil.nameOf(subclass), ClassUtil.getTypeDescription(baseType))
               );
            }
   ...
   private TypeBindings _bindingsForSubtype(JavaType baseType, int typeParamCount, Class<?> subclass, boolean relaxedCompatibilityCheck) {
      ...
      if (baseWithPlaceholders == null) {
         throw new IllegalArgumentException(
            String.format("Internal error: unable to locate supertype (%s) from resolved subtype %s", baseType.getRawClass().getName(), subclass.getName())
         );
      } else {
   ...
   private String _resolveTypePlaceholders(JavaType sourceType, JavaType actualType) throws IllegalArgumentException {
      ...
      for(int expCount = expectedTypes.size(); i < expCount; ++i) {
         ...
         if (!this._verifyAndResolvePlaceholders(exp, act)
            && !exp.hasRawClass(Object.class)
            && (i != 0 || !sourceType.isMapLikeType() || !act.hasRawClass(Object.class))
            && (!exp.isInterface() || !exp.isTypeOrSuperTypeOf(act.getRawClass()))) {
      ...
      if (rawClass == Properties.class) {
         ...
         switch(pc) {
            ...
            default:
               throw new IllegalArgumentException(
                  String.format(
                     "Strange Map type %s with %d type parameter%s (%s), can not resolve", ClassUtil.nameOf(rawClass), pc, pc == 1 ? "" : "s", bindings
                  )
               );
         }
      }
