...
public class CollectionDeserializer extends ContainerDeserializerBase<Collection<Object>> implements ContextualDeserializer {
   ...
   public Collection<Object> deserialize(JsonParser p, DeserializationContext ctxt) throws IOException {
      if (this._delegateDeserializer != null) {
         ...
         return p.hasToken(JsonToken.VALUE_STRING)
            ? this._deserializeFromString(p, ctxt, p.getText())
            : this.handleNonArray(p, ctxt, this.createDefaultInstance(ctxt));
      }
   }
   ...
   protected Collection<Object> _deserializeFromString(JsonParser p, DeserializationContext ctxt, String value) throws IOException {
      ...
      if (value.isEmpty()) {
         ...
         if (act != null) {
            return (Collection<Object>)this._deserializeFromEmptyString(p, ctxt, act, rawTargetType, "empty String (\"\")");
         }
      } else if (_isBlank(value)) {
         CoercionAction act = ctxt.findCoercionFromBlankString(this.logicalType(), rawTargetType, CoercionAction.Fail);
         return (Collection<Object>)this._deserializeFromEmptyString(p, ctxt, act, rawTargetType, "blank String (all whitespace)");
      }
   ...
   protected Collection<Object> _deserializeWithObjectId(JsonParser p, DeserializationContext ctxt, Collection<Object> result) throws IOException {
      if (!p.isExpectedStartArrayToken()) {
         ...
         JsonDeserializer<Object> valueDes = this._valueDeserializer;
         TypeDeserializer typeDeser = this._valueTypeDeserializer;
         CollectionDeserializer.CollectionReferringAccumulator referringAccumulator = new CollectionDeserializer.CollectionReferringAccumulator(
            this._containerType.getContentType().getRawClass(), result
         );
