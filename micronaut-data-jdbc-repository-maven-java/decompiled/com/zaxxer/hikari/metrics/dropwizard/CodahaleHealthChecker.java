...
public final class CodahaleHealthChecker {
   public static void registerHealthChecks(HikariPool pool, HikariConfig hikariConfig, HealthCheckRegistry registry) {
      ...
      registry.register(
         MetricRegistry.name(hikariConfig.getPoolName(), new String[]{"pool", "ConnectivityCheck"}),
         new CodahaleHealthChecker.ConnectivityHealthCheck(pool, checkTimeoutMs)
      );
      ...
      if (metricRegistry != null && expected99thPercentile > 0L) {
         SortedMap<String, Timer> timers = metricRegistry.getTimers(
            (name, metric) -> name.equals(MetricRegistry.name(hikariConfig.getPoolName(), new String[]{"pool", "Wait"}))
         );
         if (!timers.isEmpty()) {
            Timer timer = (Timer)((Entry)timers.entrySet().iterator().next()).getValue();
            registry.register(
               MetricRegistry.name(hikariConfig.getPoolName(), new String[]{"pool", "Connection99Percent"}),
               new CodahaleHealthChecker.Connection99Percent(timer, expected99thPercentile)
            );
         }
      }
   ...
   private static class Connection99Percent extends HealthCheck {
      ...
      protected Result check() throws Exception {
         ...
         return the99thPercentile <= this.expected99thPercentile
            ? Result.healthy()
            : Result.unhealthy(
               "99th percentile connection wait time of %dms exceeds the threshold %dms", new Object[]{the99thPercentile, this.expected99thPercentile}
            );
      }
   }
