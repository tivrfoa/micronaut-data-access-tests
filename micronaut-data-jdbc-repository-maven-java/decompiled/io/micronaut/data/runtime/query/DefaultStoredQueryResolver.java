...
public abstract class DefaultStoredQueryResolver implements StoredQueryResolver {
   ...
   public <E, R> StoredQuery<E, R> resolveQuery(MethodInvocationContext<?, ?> context, Class<E> entityClass, Class<R> resultType) {
      if (resultType == null) {
         resultType = (Class)context.classValue(DataMethod.NAME, "resultType").orElse(entityClass);
      }

      String query = (String)context.stringValue(Query.class).orElseThrow(() -> new IllegalStateException("No query present in method"));
      return new DefaultStoredQuery<>(context.getExecutableMethod(), resultType, entityClass, query, false, this.getHintsCapableRepository());
   }
   ...
   public <E, R> StoredQuery<E, R> resolveCountQuery(MethodInvocationContext<?, ?> context, Class<E> entityClass, Class<R> resultType) {
      ...
      return new DefaultStoredQuery<>(context.getExecutableMethod(), resultType, entityClass, query, true, this.getHintsCapableRepository());
   }

   @Override
   public <E, QR> StoredQuery<E, QR> createStoredQuery(
   ...
   @Override
   public StoredQuery<Object, Long> createCountStoredQuery(
      ExecutableMethod<?, ?> executableMethod,
      DataMethod.OperationType operationType,
      String name,
      AnnotationMetadata annotationMetadata,
