...
final class DataConversionServiceFactory {
   @Singleton
   @Bean(
      typed = {DataConversionService.class}
   )
   DataConversionServiceImpl build(@Nullable BeanContext beanContext) {
      ...
      conversionService.addConverter(Number.class, Enum.class, (TypeConverter)((index, targetType, context) -> {
      ...
      conversionService.addConverter(
         ...
         Date.class,
         localDate -> new Date(localDate.atTime(LocalTime.MIDNIGHT).atZone(ZoneId.systemDefault()).toInstant().toEpochMilli())
      );
      conversionService.addConverter(String[].class, Character[].class, values -> {
         ...
         for(int i = 0; i < values.length; ++i) {
            ...
            chars[i] = value.length() == 0 ? '\u0000' : value.charAt(0);
         }
      ...
      conversionService.addConverter(String[].class, char[].class, values -> {
         ...
         for(int i = 0; i < values.length; ++i) {
            String value = values[i];
            chars[i] = value.length() == 0 ? 0 : value.charAt(0);
         }
      ...
      conversionService.addConverter(Float[].class, BigDecimal[].class, values -> {
         ...
         for(int i = 0; i < values.length; ++i) {
            bigs[i] = BigDecimal.valueOf((double)values[i].floatValue());
         }
      ...
      conversionService.addConverter(float[].class, BigDecimal[].class, values -> {
         ...
         for(int i = 0; i < values.length; ++i) {
            bigs[i] = BigDecimal.valueOf((double)values[i]);
         }
      ...
      this.addZonedConvertorsConvertors(
         conversionService, Date.class, date -> (ZonedDateTime)instantToZonedDateTime.apply(Instant.ofEpochMilli(date.getTime()))
      );
      this.addZonedConvertorsConvertors(
         conversionService, java.sql.Date.class, date -> (ZonedDateTime)instantToZonedDateTime.apply(Instant.ofEpochMilli(date.getTime()))
      );
      ...
      if (beanContext != null) {
         for(BeanRegistration<DataTypeConverter> typeConverterRegistration : beanContext.getBeanRegistrations(DataTypeConverter.class)) {
   ...
   private Integer asInteger(Object value, DataConversionService<?> dataConversionService) {
      if (value instanceof Integer) {
         ...
         return value instanceof Number ? ((Number)value).intValue() : dataConversionService.convertRequired(value, Integer.class);
      }
   }
   ...
   private Long asLong(Object value, DataConversionService<?> dataConversionService) {
      if (value instanceof Long) {
         ...
         return value instanceof Number ? ((Number)value).longValue() : dataConversionService.convertRequired(value, Long.class);
      }
   }
   ...
   private Double asDouble(Object value, DataConversionService<?> dataConversionService) {
      if (value instanceof Double) {
         ...
         return value instanceof Number ? ((Number)value).doubleValue() : dataConversionService.convertRequired(value, Double.class);
      }
   }

   private Boolean asBoolean(Object value, DataConversionService<?> dataConversionService) {
      return value instanceof Boolean ? (Boolean)value : dataConversionService.convertRequired(value, Boolean.class);
   }
   ...
   private Float asFloat(Object value, DataConversionService<?> dataConversionService) {
      if (value instanceof Float) {
         ...
         return value instanceof Number ? ((Number)value).floatValue() : dataConversionService.convertRequired(value, Float.class);
      }
   }
   ...
   private Short asShort(Object value, DataConversionService<?> dataConversionService) {
      if (value instanceof Short) {
         ...
         return value instanceof Number ? ((Number)value).shortValue() : dataConversionService.convertRequired(value, Short.class);
      }
   }

   private Character asCharacter(Object value, DataConversionService<?> dataConversionService) {
      return value instanceof Character ? (Character)value : dataConversionService.convertRequired(value, Character.class);
   }
}
