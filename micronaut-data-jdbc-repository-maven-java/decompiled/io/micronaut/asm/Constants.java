...
final class Constants {
   ...
   static boolean isWhitelisted(String internalName) {
      if (!internalName.startsWith("io/micronaut/asm/")) {
         return false;
      } else {
         String member = "(Annotation|Class|Field|Method|Module|RecordComponent|Signature)";
         return internalName.contains("Test$")
            || Pattern.matches("io/micronaut/asm/util/Trace" + member + "Visitor(\\$.*)?", internalName)
            || Pattern.matches("io/micronaut/asm/util/Check" + member + "Adapter(\\$.*)?", internalName);
      }
   }
