...
public class InterceptorBeanMapper implements TypedAnnotationMapper<InterceptorBean> {
   ...
   public List<AnnotationValue<?>> map(AnnotationValue<InterceptorBean> annotation, VisitorContext visitorContext) {
      AnnotationValueBuilder<Annotation> builder = AnnotationValue.builder("io.micronaut.aop.InterceptorBindingDefinitions");
      AnnotationClassValue<?>[] values = annotation.annotationClassValues("value");
      AnnotationValue<?>[] bindings = new AnnotationValue[values.length];

      for(int i = 0; i < values.length; ++i) {
         bindings[i] = AnnotationValue.builder("io.micronaut.aop.InterceptorBinding").value(values[i].getName()).build();
      }
