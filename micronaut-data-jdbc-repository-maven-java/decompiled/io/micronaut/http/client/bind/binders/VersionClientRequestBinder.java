...
public class VersionClientRequestBinder implements AnnotatedClientRequestBinder<Version> {
   ...
   private ClientVersioningConfiguration getVersioningConfiguration(AnnotationMetadata annotationMetadata) {
      return (ClientVersioningConfiguration)this.versioningConfigurations
         .computeIfAbsent(
            this.getClientId(annotationMetadata),
            clientId -> (ClientVersioningConfiguration)this.beanContext
                  .findBean(ClientVersioningConfiguration.class, Qualifiers.byName(clientId))
                  .orElseGet(
                     () -> (ClientVersioningConfiguration)this.beanContext
                           .findBean(ClientVersioningConfiguration.class, Qualifiers.byName("default"))
                           .orElseThrow(
                              () -> new ConfigurationException(
                                    "Attempt to apply a '@Version' to the request, but versioning configuration found neither for '"
                                       + clientId
                                       + "' nor '"
                                       + "default"
                                       + "' provided."
