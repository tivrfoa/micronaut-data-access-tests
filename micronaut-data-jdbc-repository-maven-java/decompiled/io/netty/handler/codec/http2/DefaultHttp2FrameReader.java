               ...
               public void processFragment(boolean endOfHeaders, ByteBuf fragment, int len, Http2FrameListener listener) throws Http2Exception {
                  ...
                  if (endOfHeaders) {
                     listener.onHeadersRead(
                        ctx, headersStreamId, hdrBlockBuilder.headers(), streamDependency, weight, exclusive, padding, headersFlags.endOfStream()
                     );
                  }
      ...
      if (this.flags.ack()) {
         ...
         for(int index = 0; index < numSettings; ++index) {
            ...
            try {
               ...
               if (id == 4) {
                  throw Http2Exception.connectionError(Http2Error.FLOW_CONTROL_ERROR, var11, "Failed setting initial window size: %s", var11.getMessage());
               }

               throw Http2Exception.connectionError(Http2Error.PROTOCOL_ERROR, var11, "Protocol error: %s", var11.getMessage());
            }
         }
