...
public class DefaultHttp2Headers extends DefaultHeaders<CharSequence, CharSequence, Http2Headers> implements Http2Headers {
         ...
         if (name instanceof AsciiString) {
            ...
            try {
               ...
               PlatformDependent.throwException(
                  Http2Exception.connectionError(Http2Error.PROTOCOL_ERROR, var5, "unexpected error. invalid header name [%s]", name)
               );
               return;
            }
   ...
   public DefaultHttp2Headers(boolean validate, int arraySizeHint) {
      super(
         ...
         validate ? HTTP2_NAME_VALIDATOR : DefaultHeaders.NameValidator.NOT_NULL,
         arraySizeHint
      );
   }
   ...
   protected final DefaultHeaders.HeaderEntry<CharSequence, CharSequence> newHeaderEntry(
