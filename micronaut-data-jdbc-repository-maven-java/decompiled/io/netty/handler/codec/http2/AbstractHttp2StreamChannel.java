...
abstract class AbstractHttp2StreamChannel extends DefaultAttributeMap implements Http2StreamChannel {
   ...
   private static final AtomicLongFieldUpdater<AbstractHttp2StreamChannel> TOTAL_PENDING_SIZE_UPDATER = AtomicLongFieldUpdater.newUpdater(
      AbstractHttp2StreamChannel.class, "totalPendingSize"
   );
   private static final AtomicIntegerFieldUpdater<AbstractHttp2StreamChannel> UNWRITABLE_UPDATER = AtomicIntegerFieldUpdater.newUpdater(
      AbstractHttp2StreamChannel.class, "unwritable"
   );
   ...
   private final AbstractHttp2StreamChannel.Http2StreamChannelConfig config = new AbstractHttp2StreamChannel.Http2StreamChannelConfig(this);
   private final AbstractHttp2StreamChannel.Http2ChannelUnsafe unsafe = new AbstractHttp2StreamChannel.Http2ChannelUnsafe();
   ...
   private static final class FlowControlledFrameSizeEstimator implements MessageSizeEstimator {
      static final AbstractHttp2StreamChannel.FlowControlledFrameSizeEstimator INSTANCE = new AbstractHttp2StreamChannel.FlowControlledFrameSizeEstimator();
   ...
   private final class Http2ChannelUnsafe implements Channel.Unsafe {
      ...
      public RecvByteBufAllocator.Handle recvBufAllocHandle() {
         if (this.recvHandle == null) {
            this.recvHandle = AbstractHttp2StreamChannel.this.config().<RecvByteBufAllocator>getRecvByteBufAllocator().newHandle();
      ...
      public void close(final ChannelPromise promise) {
         if (promise.setUncancellable()) {
            if (this.closeInitiated) {
               ...
               if (AbstractHttp2StreamChannel.this.parent().isActive()
                  && !this.readEOS
                  && Http2CodecUtil.isStreamIdValid(AbstractHttp2StreamChannel.this.stream.id())) {
      ...
      private void invokeLater(Runnable task) {
         try {
            ...
            AbstractHttp2StreamChannel.logger.warn("Can't invoke task later as EventLoop rejected it", var3);
         }
      ...
      private void updateLocalWindowIfNeeded() {
         if (AbstractHttp2StreamChannel.this.flowControlledBytes != 0) {
            ...
            ChannelFuture future = AbstractHttp2StreamChannel.this.write0(
               AbstractHttp2StreamChannel.this.parentContext(), new DefaultHttp2WindowUpdateFrame(bytes).stream(AbstractHttp2StreamChannel.this.stream)
            );
      ...
      void notifyReadComplete(RecvByteBufAllocator.Handle allocHandle, boolean forceReadComplete) {
         if (AbstractHttp2StreamChannel.this.readCompletePending || forceReadComplete) {
            ...
            if (AbstractHttp2StreamChannel.this.readStatus == AbstractHttp2StreamChannel.ReadStatus.REQUESTED) {
               ...
               AbstractHttp2StreamChannel.this.readStatus = AbstractHttp2StreamChannel.ReadStatus.IDLE;
            }
      ...
      public void write(Object msg, ChannelPromise promise) {
         if (!promise.setUncancellable()) {
            ReferenceCountUtil.release(msg);
         } else if (AbstractHttp2StreamChannel.this.isActive()
            && (!AbstractHttp2StreamChannel.this.outboundClosed || !(msg instanceof Http2HeadersFrame) && !(msg instanceof Http2DataFrame))) {
            try {
      ...
      private void writeHttp2StreamFrame(Http2StreamFrame frame, final ChannelPromise promise) {
         if (!AbstractHttp2StreamChannel.this.firstFrameWritten
            && !Http2CodecUtil.isStreamIdValid(AbstractHttp2StreamChannel.this.stream().id())
            && !(frame instanceof Http2HeadersFrame)) {
            ReferenceCountUtil.release(frame);
