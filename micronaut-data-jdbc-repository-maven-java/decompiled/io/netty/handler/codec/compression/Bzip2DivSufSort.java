...
final class Bzip2DivSufSort {
   ...
   private void lsSort(int isa, int n, int depth) {
      ...
      for(int isaD = isa + depth; -n < SA[0]; isaD -= isa) {
         int first = 0;
         int skip = 0;
   ...
   private int sortTypeBstar(int[] bucketA, int[] bucketB) {
      ...
      if ((ti = T[i] & 255) < (t0 = T[0] & 255) || T[i] == T[0] && flag != 0) {
         if (flag == 0) {
            int var10002 = bucketB[BUCKET_BSTAR(ti, t0)]++;
            m = n - 1;
            SA[m] = i;
         } else {
            int var52 = bucketB[BUCKET_B(ti, t0)]++;
         }
         ...
         while(0 <= i && (ti = T[i] & 255) <= (ti1 = T[i + 1] & 255)) {
            int var53 = bucketB[BUCKET_B(ti, ti1)]++;
            --i;
         }
      }
      ...
      while(0 <= i) {
         do {
            int var54 = bucketA[T[i] & 255]++;
            --i;
         } while(0 <= i && (T[i] & 255) >= (T[i + 1] & 255));

         if (0 <= i) {
            int var55 = bucketB[BUCKET_BSTAR(T[i] & 255, T[i + 1] & 255)]++;
            --m;

            int ti1;
            for(SA[m] = i--; 0 <= i && (ti = T[i] & 255) <= (ti1 = T[i + 1] & 255); --i) {
               var55 = bucketB[BUCKET_B(ti, ti1)]++;
            }
         }
      }
