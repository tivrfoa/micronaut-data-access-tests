...
public final class DefaultHostsFileEntriesResolver implements HostsFileEntriesResolver {
   ...
   public List<InetAddress> addresses(String inetHost, ResolvedAddressTypes resolvedAddressTypes) {
      ...
      switch(resolvedAddressTypes) {
         case IPV4_ONLY:
            return (List<InetAddress>)this.inet4Entries.get(normalized);
         case IPV6_ONLY:
            return (List<InetAddress>)this.inet6Entries.get(normalized);
         case IPV4_PREFERRED:
            List<InetAddress> allInet4Addresses = (List)this.inet4Entries.get(normalized);
            return allInet4Addresses != null
               ? allAddresses(allInet4Addresses, (List<InetAddress>)this.inet6Entries.get(normalized))
               : (List)this.inet6Entries.get(normalized);
         case IPV6_PREFERRED:
            List<InetAddress> allInet6Addresses = (List)this.inet6Entries.get(normalized);
            return allInet6Addresses != null
               ? allAddresses(allInet6Addresses, (List<InetAddress>)this.inet4Entries.get(normalized))
               : (List)this.inet4Entries.get(normalized);
         default:
