...
public final class ByteBufUtil {
   ...
   static int firstIndexOf(AbstractByteBuf buffer, int fromIndex, int toIndex, byte value) {
      ...
      if (fromIndex < toIndex && buffer.capacity() != 0) {
         ...
         if (!PlatformDependent.isUnaligned()) {
            ...
            long pattern = ByteBufUtil.SWARByteSearch.compilePattern(value);

            for(int i = 0; i < longCount; ++i) {
               long word = useLE ? buffer._getLongLE(offset) : buffer._getLong(offset);
               int index = ByteBufUtil.SWARByteSearch.firstAnyPattern(word, pattern, isNative);
               if (index < 8) {
                  return offset + index;
               }
   ...
   private static int safeDirectWriteUtf8(ByteBuffer buffer, int writerIndex, CharSequence seq, int start, int end) {
      ...
      for(int i = start; i < end; ++i) {
         ...
         if (c < 128) {
            ...
            buffer.put(writerIndex++, (byte)(128 | c & '?'));
   ...
   private static int unsafeWriteUtf8(byte[] buffer, long memoryOffset, int writerIndex, CharSequence seq, int start, int end) {
      ...
      for(int i = start; i < end; ++i) {
         ...
         if (c < 128) {
            ...
            PlatformDependent.putByte(buffer, writerOffset++, (byte)(128 | c & '?'));
         } else if (StringUtil.isSurrogate(c)) {
            if (!Character.isHighSurrogate(c)) {
   ...
   private static final class SWARByteSearch {
      ...
      private static int firstAnyPattern(long word, long pattern, boolean leading) {
         ...
         tmp = ~(tmp | input | 9187201950435737471L);
   ...
   static final class ThreadLocalDirectByteBuf extends UnpooledDirectByteBuf {
      private static final ObjectPool<ByteBufUtil.ThreadLocalDirectByteBuf> RECYCLER = ObjectPool.newPool(
         new ObjectPool.ObjectCreator<ByteBufUtil.ThreadLocalDirectByteBuf>() {
      ...
      private final ObjectPool.Handle<ByteBufUtil.ThreadLocalDirectByteBuf> handle;
   ...
   static final class ThreadLocalUnsafeDirectByteBuf extends UnpooledUnsafeDirectByteBuf {
      private static final ObjectPool<ByteBufUtil.ThreadLocalUnsafeDirectByteBuf> RECYCLER = ObjectPool.newPool(
         new ObjectPool.ObjectCreator<ByteBufUtil.ThreadLocalUnsafeDirectByteBuf>() {
