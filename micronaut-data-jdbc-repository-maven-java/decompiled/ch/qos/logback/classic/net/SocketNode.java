...
public class SocketNode implements Runnable {
   ...
   public void run() {
      try {
         ...
         this.logger.error("Could not open ObjectInputStream to " + this.socket, var4);
         this.closed = true;
      }
      ...
      try {
         ...
         this.logger.error("Unexpected exception. Closing connection.", var8);
      }
      ...
      if (!this.closed) {
         ...
         if (this.hardenedLoggingEventInputStream != null) {
            try {
               ...
               this.logger.warn("Could not close connection.", var5);
            } finally {
